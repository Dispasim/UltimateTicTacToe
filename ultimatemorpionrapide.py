# -*- coding: utf-8 -*-
"""
Created on Fri May  5 17:52:22 2023
...
def Max_Value(jeu,alpha,beta,joueur,profondeur1,profondeurmax,t1):
    ...
    profondeur += 1
    v=jeu.heuristique9(joueur)
    
    if jeu.fini():
        return jeu.utility(joueur)
    
    elif profondeur==profondeurmax or time.time()-t1>9.9:
        return v
...
def Partie(commence : bool, joueur : int, ia : bool):
    jeu = grandPlateau()
    action=[0,0]
    if commence:
    ...
    if not ia:
        ...
        liste_temps = []
        while True:
            action,temps  = Min_Max_Decision(jeu, 5, -joueur)
